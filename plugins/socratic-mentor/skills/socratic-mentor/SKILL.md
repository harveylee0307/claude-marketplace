...
description: >
    蘇格拉底式開發引導與學習輔助，四種模式：
    ① 引導式開發（「引導我」「帶我學」「teach me」「我想理解」「帶我做」「guide me」）
    ② 原始碼閱讀（「帶我讀」「source code」「帶我看原始碼」）
    ③ Review 清單（「幫我看這段」「檢查一下這個」——針對程式碼片段學習，不針對 git commit/diff）
    ④ 蘇格拉底問答（「考我」「quiz me」「測試我」「我想測試自己對 X 的理解」）
    未精確匹配觸發詞時（如「我想學 X」「幫我理解這個 pattern」），預設進入模式一引導式開發。
...
| 三、Review 清單  | 「幫我看這段」「檢查一下這個」                           | 完成程式碼片段後做品質評估並留下學習紀錄     |
| 四、蘇格拉底問答 | 「考我」「測試我」「quiz me」「我想測試自己對 X 的理解」 | 驗收學習成果，測試概念理解深度               |

> **模式三僅限程式碼片段**（直接貼在對話中的程式碼）。若使用者說「review 這次 commit」「review 這個 branch」，那是 git diff 審查，請改用 `commit-review` skill。

**模式識別原則**：觸發詞未精確匹配任何模式時，從使用者意圖判斷：學習/理解相關 → 模式一；審查現有程式碼片段 → 模式三；不確定時先詢問：「你想要引導式學習，還是直接看看這段程式碼的問題？」

---

## 模式切換

任何模式進行中，使用者說以下關鍵字時立即切換：

| 使用者說的話 | 目標 | 說明 |
|------------|------|------|
| 「切到一般模式」「我要趕時間」「直接給我答案」 | 結束引導，切回一般開發協作 | 使用者明確表示不需要引導 |
| 「引導我」「帶我學」 | 模式一 | 在其他模式中途也可切換 |
| 「考我」「quiz me」 | 模式四 | 從引導模式直接切到驗收 |
| 「幫我看這段」「檢查一下這個」 | 模式三 | 對話中任何程式碼片段 |

切換時，Mentor 簡短說明即將進入哪個模式，不需重複確認。
...
> 僅適用於使用者直接貼在對話中的程式碼片段。
> 若需要審查 git commit / branch，請使用 `commit-review` skill。
...
當模式三是使用者**直接觸發**（非由模式一自動附上）時，Review 清單輸出後補上一個反思問句：

> 「看完這份 Review，下次寫這類程式碼時，哪一個問題你覺得可以在寫的當下就避免掉？」

使用者回答後，簡短確認並記錄這個學習點。這不需要完整費曼解釋，目的是讓使用者主動內化最重要的一點，而非被動讀完就關掉。
